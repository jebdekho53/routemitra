// Sample / dummy data — meant to have the SAME shape a real aggregator
// backend would return (see the "normalization schema" in the RouteMitra
// blueprint). Swap fetchRoute() below for a real API call later and the
...
// Simulates an async aggregator call (bus API + train API + flight API
// fired in parallel, results merged) — real version replaces the body,
// keeps the same return shape: { from, to, options[] } | null
function fetchRoute(from, to) {
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      resolve(null);
    }, 380); // fake network latency so the loading state is visible
  });
}

function listSampleRoutes() {
